...
class PythonHoster(Hoster):
    ...
    - ``python_bin``: use a specific python binary for the venv.
    - ``skip_deps``: skip pip install (deps already installed by caller).
    - ``entry_point``: explicit ASGI/WSGI module (e.g. ``myapp.main:app``).
    - ``extra_env``: extra Environment directives for the systemd unit.
    """

    name = "Python"
    aliases = ("python",)
    description = "Python web applications (WSGI/ASGI)"
    priority = 20
